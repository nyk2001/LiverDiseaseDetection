...
The variable "Albumin_and_Globulin_Ratio" has four null values. We replace null values with the mean of the variable as done commonly in data science.
...
These proteins are crucial for body growth, development, and health. They form the structural part of most organs and makeup enzymes and hormones that regulate body functions. The low ratios refer to liver issues, and we can notice the same from distributions graphs. 
...
\section{Results}
\label{sec:results}
In the Section \ref{sec:methods}, we trained a number of models using training data. Now, we will evaluate the performance of these models using validation data. We will not use \textbf{lda} model due to poor performance.

```{r}
y_hat<-predict(train_glm, validation) 
sensitivity(data = y_hat, reference = validation$LiverDisease)
specificity(data = y_hat, reference = validation$LiverDisease)
F_meas(data = y_hat, reference = validation$LiverDisease)
cm<-confusionMatrix(y_hat,validation$LiverDisease)
cm$overall['Accuracy']
cm$byClass[c("Sensitivty","Specificity")]

#y_hat<-predict(train_knn, validation) 
#sensitivity(data = y_hat, reference = validation$LiverDisease)
#specificity(data = y_hat, reference = validation$LiverDisease)
#F_meas(data = y_hat, reference = validation$LiverDisease)
